	...
	setup( props, { emit } ) {
		...
		function getImageSources( image ) {
			...
			const projectsMap = ( image.externalSources || [] ).reduce(
				( map, hostname ) => {
					const domain = Object.keys( externalWikis ).find( ( wiki ) => hostname.endsWith( wiki ) );
					if ( domain ) {
			...
			const highlightIndex = btoa( wikipedias ).charCodeAt( wikipedias.length ) % wikipedias.length;
			return $i18n(
