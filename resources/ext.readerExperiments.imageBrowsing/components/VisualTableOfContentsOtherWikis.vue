...
<script>
const { defineComponent, nextTick, ref, useTemplateRef } = require( 'vue' );
...
module.exports = exports = defineComponent( {
	...
	setup( props, { emit } ) {
		...
		const insersectionObserver = new IntersectionObserver(
			async ( entries ) => {
				if ( !entries[ 0 ].isIntersecting ) {
					// Don't start loading images until they're intersecting
					// with the viewport
					return;
				}

				if ( done.value === true || images.value.length > 0 ) {
					// Load only once
					return;
				}
				...
				try {
					...
					images.value = await useExternalImages();
				} catch ( err ) {
					// eslint-disable-next-line no-console
		...
		nextTick( () => {
			insersectionObserver.observe( containerElement.value );
		} );
